...
{
	if (snakeLength[number] <= 2)
	{
		winner = 1 - number;
		snakeLength[number] = 1;
		return true;
	}
	if (snakeLength[number] > 80)
	{
		winner = number;
		return true;
	}
...
bool SnakeEatSnake()
{
	...
	for (int j = 0; j < 2; ++j)
	{
		for (int i = 1; i < snakeLength[j]; ++i)
		{
			if (Snake[j][0].x == Snake[j][i].x  && Snake[j][0].y == Snake[j][i].y && i!= 0)
			{
				winner = abs(1 - j);
				return true;
			}
...
bool Prediction(int direction, int snakeNum)
{
	for (int j = 1; j < snakeLength[1 - snakeNum]; ++j)
	{
		if (Snake[1 - snakeNum][j].x == Snake[snakeNum][0].x)
		{
			if (abs(Snake[1 - snakeNum][j].y - Snake[snakeNum][0].y) < 80)
				return true;
		}
		if (Snake[1 - snakeNum][j].y == Snake[snakeNum][0].y)
		{
			if (abs(Snake[1 - snakeNum][j].y - Snake[snakeNum][0].y) < 80)
			return true;
		}
